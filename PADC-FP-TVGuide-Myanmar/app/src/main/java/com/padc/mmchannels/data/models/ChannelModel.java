...
//        loadChannelDetails();
    ...
    public void notifyChannelsLoaded(List<ChannelVO> channelList) {
        Log.e(MMChannelsApp.TAG, "ChannelModel.notifyChannelsLoaded.channelList.size:" + channelList.size());
...
//        broadcastChannelLoadedWithLocalBroadcastManager();
    ...
    public void notifyChannelDetailsLoaded(ChannelDetailsVO channelDetails) {
        Log.e(MMChannelsApp.TAG, "ChannelModel.notifyChannelDetailsLoaded.channelDetails.channel:" + channelDetails.getChannel().getChannel_name());
...
//        broadcastChannelDetailsLoadedWithLocalBroadcastManager();
    ...
    public void notifyErrorInLoadingChannels(String message) {
		...
		Log.e(MMChannelsApp.TAG, "ChannelModel.notifyErrorInLoadingChannels.message:" + message);
    }

    public void notifyErrorInLoadingChannelDetails(String message) {
        broadcastConnectionErrorWithEventBus();
        Log.e(MMChannelsApp.TAG, "ChannelModel.notifyErrorInLoadingChannelDetails.message:" + message);
    }

    private void broadcastChannelLoadedWithLocalBroadcastManager() {
		Log.e(MMChannelsApp.TAG, "broadcastAttractionLoadedWithLocalBroadcastManager");
        Intent intent = new Intent(BROADCAST_DATA_LOADED);
        intent.putExtra("key-for-extra", "extra-in-broadcast");
        LocalBroadcastManager.getInstance(MMChannelsApp.getContext()).sendBroadcast(intent);
    }
